...
const toU8 = (a: Hex, len?: number) => abytes(isS(a) ? hexToBytes(a) : u8fr(abytes(a)), len); // norm(hex/u8a) to u8a
...
): bigint => (isB(n) && min <= n && n < max ? n : err(msg));
...
const bytesToNum_LE = (b: Bytes): bigint => BigInt('0x' + bytesToHex(u8fr(abytes(b)).reverse()));
